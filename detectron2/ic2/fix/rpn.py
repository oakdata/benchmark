...
class RPN(nn.Module):
    ...
    def losses(
        ...
        Args:
            ...
            cm_ratio: plus the positive part of memory loss with the ratio and double the negative loss from origin
        ...
        # need to plus cm_ratio and separate the negative and positive, twice the negative
        # print(len(pred_objectness_logits),pred_objectness_logits[0].size())
 
        total_obj = 0
        for i,label in enumerate(labels):
            pos_mask = gt_labels[i:i+1] > 0
            objectness_loss = F.binary_cross_entropy_with_logits(
                    cat(pred_objectness_logits, dim=1)[i:i+1][pos_mask],
                    gt_labels[i:i+1][pos_mask].to(torch.float32),
                    reduction="sum",
                )
            cur_obj_loss = objectness_loss/self.batch_size_per_image

            if label == 0: # current
                total_obj += cur_obj_loss
            else: # memory
                total_obj += cur_obj_loss * cm_ratio


            neg_mask = gt_labels[i:i+1] == 0
            if labels == 0: # current
                objectness_loss = F.binary_cross_entropy_with_logits(
                    cat(pred_objectness_logits, dim=1)[i:i+1][neg_mask],
                    gt_labels[i:i+1][neg_mask].to(torch.float32),
                    reduction="sum",
                )
                cur_obj_loss = objectness_loss/self.batch_size_per_image
                if cm_ratio > 0 and cm_ratio < 1:
                    total_obj += cur_obj_loss * 2
                else:
                    total_obj += cur_obj_loss
        ...
        return {
            "loss_rpn_cls": total_obj / num_images,
    ...
    def forward(
        ...
        Args:
            ...
            labels: this batch is from memory(1) or current frame(0)
        ...
        if self.training:
            ...
            losses = self.losses(
                anchors, pred_objectness_logits, gt_labels, pred_anchor_deltas, gt_boxes,labels, cm_ratio
            )
