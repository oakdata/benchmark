...
def build_idk(cfg,num_oc,wp):
    ...
    if 'pixel_mean' in w.keys() and 'pixel_mean' not in model.state_dict():
        w.pop('pixel_mean')
        w.pop('pixel_std')

    if 'proposal_generator.anchor_generator.cell_anchors.0' in model.state_dict():
        if 'proposal_generator.anchor_generator.cell_anchors.0' not in w:
            w['proposal_generator.anchor_generator.cell_anchors.0'] = tmpw['proposal_generator.anchor_generator.cell_anchors.0']
    else:
        if 'proposal_generator.anchor_generator.cell_anchors.0' in w:
            w.pop('proposal_generator.anchor_generator.cell_anchors.0')
...
def main(args):
    ...
    for name in files:
        if 'model' in name:
            model_names.append(name) #　str(count)zfill(5)_model.pth
    ...
    for name in model_names:
        ...
        t_train_map = json.load(open(osp.join(model_dir,str(count).zfill(5) + '_trainmap.json'),'r'))
        train_map = {}
        ...
        cfg.MODEL.IDKTHRESH = (curtt,curit,curbt)
        cfg.DATASETS.TEST = ('oak_test',)
        cfg.SOLVER.IMS_PER_BATCH = 16
        wp = osp.join(model_dir,name)


        cfg,model = build_idk(cfg,len(train_map),wp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tt', default=0.1)
    parser.add_argument('--it', default=0.1)
    parser.add_argument('--bt', default=0.6)
